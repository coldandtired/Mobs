...
public class Condition extends Config_element
{
	...
	public Condition(Element element, Config_element parent) throws XPathExpressionException 
	{
		...
		condition_type = MCondition.valueOf(element.getAttribute("type").toUpperCase());
		
		values = fillString_list("values", element);
		mobs = fillString_list("mobs", element);
	}
	...
	public boolean matches(Condition_report cr, int orig)
	{
		Mobs.log(orig);
		Mobs.log(cr.toString());
		cr.setCheck_value(values, true);
		cr.setActual_value(orig);
		return matches(orig);
	}
	...
	{
		cr.setCheck_value(values, false);
		cr.setActual_value(orig);
	...
	{
		boolean b = condition_type.equals(cond);
		cr.setCheck_value(values, b);
		cr.setActual_value(orig);
		if (values == null) return !b;
		return values.contains(orig) == b;
	}
